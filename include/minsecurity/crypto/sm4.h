...
//         }
    ...
    static std::vector<uint8_t> EncryptCBCNoPadding(const std::vector<uint8_t>&  key,const std::vector<uint8_t>& iv,const std::vector<uint8_t>& data){
        static  SM4_KEY enc_key;
        std::vector<uint8_t> res;
    ...
    static std::vector<uint8_t> DecryptCBCNoPadding(const std::vector<uint8_t>&  key,const std::vector<uint8_t>& iv,const std::vector<uint8_t>& data){
        static  SM4_KEY dec_key;
        std::vector<uint8_t> res;
        if(key.size() != SM4_BLOCK_SIZE || iv.size() != SM4_BLOCK_SIZE || data.size() % SM4_BLOCK_SIZE != 0 ){
            return res;
        }
        sm4_set_decrypt_key(&dc_key, key.data());
        res.resize(data.size());
    ...
    static std::vector<uint8_t> EncryptECBNoPadding(const std::vector<uint8_t>&  key,const std::vector<uint8_t>& data){
        ...
        sm4_set_encrypt_key(&enc_key, key.data());
        res.resize(data.size());
        sm4_encrypt_blocks(&enc_key, iv.data(), data.data(), data.size() / SM4_BLOCK_SIZE, res.data());
        return res;
    }
    static std::vector<uint8_t> DecryptECBNoPadding(const std::vector<uint8_t>&  key,const std::vector<uint8_t>& data){
        ...
        sm4_set_decrypt_key(&dc_key, key.data());
        res.resize(data.size());
        sm4_encrypt_blocks(&dec_key, iv.data(), data.data(), data.size() / SM4_BLOCK_SIZE, res.data());
        return res;
    }
    static std::vector<uint8_t> EncryptECBPadding(const std::vector<uint8_t>&  key,const std::vector<uint8_t>& data){
        ...
        res.resize((data.size()/SM4_BLOCK_SIZE+1)*SM4_BLOCK_SIZE);
        size_t outlen = 0;
        if(sm4_ecb_encrypt_update(&ctx,data.data(),res.data(),&outlen) != 1 || sm4_ecb_encrypt_finish(&ctx,data.data(),&outlen)!=1){
            error_print();
            res.clear();
        }
        return std::vector<uint8_t>(res.begin(),res.begin()+outlen);
    }
    static std::vector<uint8_t> DecryptECBPadding(const std::vector<uint8_t>&  key,const std::vector<uint8_t>& iv,const std::vector<uint8_t>& data){
        static SM4_ECB_CTX ctx;
        std::vector<uint8_t> res;
        if(key.size() != SM4_BLOCK_SIZE){
            return res;
        ...
        res.resize((data.size()/SM4_BLOCK_SIZE+1)*SM4_BLOCK_SIZE);
        size_t outlen = 0;
        if(sm4_ecb_decrypt_update(&ctx,data.data(),res.data(),&outlen) != 1 || sm4_ecb_decrypt_finish(&ctx,data.data(),&outlen)!=1){
            error_print();
            res.clear();
        }
        return std::vector<uint8_t>(res.begin(),res.begin()+outlen);
