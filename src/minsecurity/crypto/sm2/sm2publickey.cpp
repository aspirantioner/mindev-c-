...
namespace mindev::minsecurity::crypto::sm2 {
    ...
    std::vector<char> SM2PublicKey::Encrypt(const std::vector<char>& content,Common::Sm2CipherMode cipher_mode){
        std::vector<char> res;
        ...
        res.insert(res.end(),reinterpret_cast<char*>(ciphertext.point.x),reinterpret_cast<char*>(ciphertext.point.x)+sizeof(ciphertext.point.x));
        res.insert(res.end(),reinterpret_cast<char*>(ciphertext.point.y),reinterpret_cast<char*>(ciphertext.point.y)+sizeof(ciphertext.point.y));
    ...
    bool SM2PublicKey::Verify(const std::vector<char>& content,const std::vector<char>& digest){
        ...
        SM2_SIGNATURE sign;
        memcpy(sign.r, digest.data(), sizeof(sign.r));
        memcpy(sign.s, digest.data()+sizeof(sign.r), sizeof(sign.s));
