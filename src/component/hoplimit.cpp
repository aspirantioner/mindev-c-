...
namespace mindev::component{
    int HopLimit::WireEncode(mindev::encoding::Encoder& encoder){
        int totalLength = 0;
        ...
        totalLength+=tmpLen;
        tmpLen = encoder.PrependVarNumber(mindev::encoding::TLV::TlvHopLimit);
        if(tmpLen<0){
            return -1;
        }
        totalLength+=tmpLen;
    ...
    bool HopLimit::WireDecode(mindev::encoding::Block& block){
        if(!mindev::encoding::TLV::ExpectType(block.GetType(), mindev::encoding::VlInt(mindev::encoding::TLV::TlvHopLimit))){
            return false;
        }
